    ...
    with torch.no_grad():
        for i, (L_batch, ab_batch) in enumerate(test_loader):
            ...
            for j in range(L_batch.size(0)):
                ...
                psnr_value = psnr(rgb_gt, rgb_pred, data_range=1.0)
                ssim_value = ssim(rgb_gt, rgb_pred, multichannel=True, data_range=1.0)
...
def transfer_learning_on_ncdataset(model, device, gray_dir, color_dir, num_epochs=10, batch_size=10, initial_lr=1e-4, unfreeze_interval=3):
    ...
    # Include augmented images in the dataset
    augmented_image_paths = get_image_paths_from_directory(augmented_dir)

    # Duplicate grayscale images only for the number of augmented images
    gray_image_paths.extend(gray_image_paths[:len(augmented_image_paths)])
    color_image_paths.extend(augmented_image_paths)

    # Ensure gray_image_paths and color_image_paths have the same length
    min_length = min(len(gray_image_paths), len(color_image_paths))
    gray_image_paths = gray_image_paths[:min_length]
    color_image_paths = color_image_paths[:min_length]

    # Split into training (90%) and testing (10%) datasets
    train_size = int(0.9 * len(color_image_paths))
    test_size = len(color_image_paths) - train_size

    print(f"Total images: {len(color_image_paths)}")
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > 0 and epoch % unfreeze_interval == 0:
            ...
            optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=initial_lr * 0.1)
            print(f"Epoch {epoch}: Unfroze {num_layers_to_unfreeze} layers. Adjusted learning rate to {initial_lr * 0.1}")
...
# Grab the paired image paths from their respective directories
...
def main():
    ...
    freeze_layers(model, num_layers_to_freeze=0)
